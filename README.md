# マーケット用語ターミナル

市場系システムエンジニア向けの金融・市場用語集＋クイズツールです。証券取引・注文執行・清算決済・デリバティブ・為替・債券・規制など、市場系システムの開発・保守で押さえておきたい用語をまとめています。

## オンラインで試す

以下のURLで、このリポジトリの内容をブラウザ上でそのまま試せます（サーバー不要）。GitHub Pagesで公開しています。

https://shuya0908.github.io/market-terms-glossary/

`main` ブランチに変更がpushされると、GitHub Actions（`.github/workflows/pages.yml`）により自動的に上記URLへ反映されます。

## できること

- **用語集の閲覧・検索**: 用語名・ふりがな・定義文から全文検索できます。カテゴリごとに一括選択／解除も可能です。
- **カテゴリ分類**: 取引・板情報、注文・執行、市場制度・取引所、清算・決済、デリバティブ、システム関連、規制・コンプライアンス、フロント／ミドル／バックオフィス、為替取引（FX）、債券・マネーマーケット、クオンツ・プライシングモデル、リスク指標・ポートフォリオ理論、担保・与信管理、ALM・銀行勘定金利リスクの各カテゴリに整理しています。
- **図解タブ**: 板の読み方、注文が決済に至るまでの流れ、オプションの損益、証拠金と値洗い、イールドカーブ、レポ取引の資金・証券フローなど、文章だけでは像を結びにくい仕組みを図解する専用タブです。
- **修正履歴タブ**: 用語集本体に載っている既存用語の定義修正や、それに伴う関連用語の追加を、日付つきで振り返れる専用タブです（`changelog.json` から読み込みます）。用語集タブには常に現在の内容だけが表示されるため、何をいつ・なぜ直したかはこちらで確認できます。
- **クイズ機能**: カテゴリや出題数を指定してランダム出題されるクイズです。「定義→用語」「用語→定義」の4択に加えて、用語の説明文中の単語が空欄になり解答者がそれを記述して埋める「穴埋め（記述式）」形式も選べます。「ランダム混在」を選ぶとこの3形式が問題ごとにランダムに切り替わります。4択では解答後に正解・不正解にかかわらずすべての選択肢の解説を確認でき、穴埋め形式では正解の単語が表示されます。「中断する」ボタンでいつでも設定画面に戻れます（中断した回はクイズ成績には記録されません）。
- **マーク（ウォッチリスト）機能**: 気になる用語をマークして後から一覧できます。同じブラウザの複数タブ間でもマーク状態がリアルタイムに同期します。
- **管理タブ**: 用語の追加・編集・削除は「管理」タブに集約しています。組み込みの用語を含め、すべての用語をその場で追加・編集・削除できます（変更内容はブラウザのlocalStorageに保存されます）。用語集・マーク済みタブは閲覧・検索・マーク専用で、編集操作は行いません。
- **バックアップの保存・復元**: マーク・クイズの成績・追加した用語をJSONファイルとして保存し、後から読み込んで復元できます。
- **ホーム画面への追加（PWA）**: iPhoneのSafariなどから「ホーム画面に追加」すると、URLバーなしのアプリのような見た目で起動できます。マーク・クイズ成績・追加した用語はブラウザのlocalStorageに保存されるため、ホーム画面から開いた場合も保持されます（端末をまたいだ同期はされません。上記のバックアップ機能をご利用ください）。

## 使い方

用語データは `terms.json` に分離されており、`index.html` が読み込み時に `fetch` で取得します。そのため `file://` で直接開くとブラウザのCORS制限により `terms.json` を読み込めません。`index.html` と `terms.json` を同じフォルダに置いたまま、簡易HTTPサーバーを起動して開いてください（ビルド環境は不要です）。

```
python3 -m http.server 8000
```

その後ブラウザで `http://localhost:8000/index.html` を開きます。

## 収録内容

現時点で用語数は289語です。用語は今後も追加・修正されます。

## 技術構成

HTML / CSS / JavaScript のみで構成されたWebアプリケーションで、ビルド環境は不要です。用語データは `terms.json` に分離しており、`index.html` が起動時に `fetch` で読み込みます。外部ライブラリへの依存はGoogle Fonts（Zen Kaku Gothic New, Noto Sans JP, IBM Plex Mono）のみです。`manifest.json` と `icon.png` により、iPhone等での「ホーム画面に追加」に対応しています。GitHub Pagesへの公開は `.github/workflows/pages.yml` のGitHub Actionsワークフローが行います。

## ファイル構成

- `index.html` — 画面表示・検索・クイズ・マーク・バックアップなどのロジック一式
- `terms.json` — 組み込みの用語データ（用語・ふりがな・カテゴリ・定義）
- `changelog.json` — 既存用語の修正・関連用語追加の履歴データ（修正履歴タブが表示）
- `manifest.json` — ホーム画面追加（PWA）用のWeb App Manifest
- `icon.png` — ホーム画面アイコン
- `.github/workflows/pages.yml` — GitHub Pagesへの自動デプロイ用ワークフロー

## 免責事項

本ツールに掲載する用語の定義は学習・参考目的の一般的な解説であり、正確性を保証するものではありません。実務上の判断には、各取引所・金融機関等が公表する公式資料をご確認ください。
